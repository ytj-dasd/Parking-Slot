...
void  Ground_Extraction::Get_grid(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
	...
	float non_ground_z_min = std::numeric_limits<float>::max();
...
void Ground_Extraction::Seg_ground_nground_pts(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
...
void Ground_Extraction::Extract_ground_pts(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
	                                       pcl::PointCloud<pcl::PointXYZRGB>::Ptr ground_cloud,
	                                       pcl::PointCloud<pcl::PointXYZRGB>::Ptr no_ground_cloud,
	...
	for (int i = 0; i < num_voxel; i++) {
		grid[i].min_z = std::numeric_limits<float>::max();;
	}
