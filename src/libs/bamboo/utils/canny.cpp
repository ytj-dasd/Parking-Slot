...
void conv2D(const cv::Mat& src, cv::Mat& dst, cv::Mat kernel, int ddepth, 
        cv::Point anchor, int delta, int borderType) {
	cv::Mat  kernelFlip;
...
//Canny边缘检测
void Canny(const cv::Mat &src, cv::Mat &edge, float TL, float TH, 
        EdgeAngles angles, int wsize, bool L2graydient) {
	int rows = src.rows;
	int cols = src.cols;
	...
	for (int r = 1; r < rows - 1; ++r) {
		for (int c = 1; c < cols - 1; ++c) {
			...
			if ((angles & EDGE_ANGLE_90) 
                    && (abs(angle) < 22.5 || abs(angle) > 157.5)) {
